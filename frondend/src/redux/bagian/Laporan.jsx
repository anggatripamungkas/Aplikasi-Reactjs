...
export const updateReport = createAsyncThunk(
  ...
  async ({ _id, location, description }, { rejectWithValue }) => {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.put(
        `http://localhost:5000/api/places/${_id}`,  // Gunakan _id di URL
        { location, description },  // Hanya kirimkan location dan description
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateReport.fulfilled, (state, action) => {
        console.log("Data yang diterima di reducer untuk update:", action.payload);
