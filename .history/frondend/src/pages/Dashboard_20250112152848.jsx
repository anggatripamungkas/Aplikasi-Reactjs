...
const Dashboard = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Jika editReportId ada, pastikan kita menggunakan nilai tersebut
    if (editReportId) {
      const dataToSend = { _id: editReportId, ...formData };  // Gabungkan _id dengan formData
      console.log("Form Data untuk Update:", dataToSend);
      dispatch(updateReport(dataToSend));  // Kirim data untuk update
    } else {
      dispatch(createReport(formData));  // Kirim data untuk membuat laporan baru
    }
  
    setFormData({ location: "", description: "" });
    setEditReportId(null); // Reset setelah submit
  };
